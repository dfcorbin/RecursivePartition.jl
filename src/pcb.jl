...
function rescale_vec!(x::Vector{Float64}, x1::Vector{Float64}, kmat::Matrix{Float64})
    for i = 1:length(x)
...
function univar_array(X::Matrix{Float64}, degmax::Int64, kmat::Matrix{Float64})
    ...
    for obs = 1:N, deg = 3:(degmax+1), d = 1:dim
        out[obs, d, deg] =
            legendre_next(deg - 1, out[obs, d, 2], out[obs, d, deg-1], out[obs, d, deg-2])
    end
    return out
end
...
function find_mvpindex(
    d::Int64,
    degmax::Int64,
    root::Bool = true,
    d0::Int64 = d,
    index::MVPIndex = MVPIndex(),
...
```
"""
function index_pcbmat(X::Matrix{Float64}, indices::Vector{MVPIndex}, kmat::Matrix{Float64})
    N = size(X, 1)
    dim1 = length(indices)
    out = ones(Float64, N, dim1)
    degmax = maximum([maximum(indices[i].deg) for i = 1:dim1])
